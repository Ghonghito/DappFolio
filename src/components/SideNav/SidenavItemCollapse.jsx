  ...
  const renderCollapse = collapse.map(({ name, key, path }) => {
    ...
    returnValue = (
      <NavLink key={key} exact='true' to={path}>
        <div className='duration-150 hover:bg-white dark:hover:bg-darkCard rounded-lg p-2 cursor-pointer'>
          <p className={`text-lightText dark:text-darkText duration-150 text-sm ${path === pathname ? 'bg-lightHover dark:bg-darkCard p-2 rounded-lg' : ''}`}>{name}</p>
        </div>
      </NavLink>
